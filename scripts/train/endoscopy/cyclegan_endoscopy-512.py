...
def parse_cmdargs():
    parser = argparse.ArgumentParser(
        description = 'Train endoscopy CycleGANs'
    )
...
def get_transfer_preset(cmdargs):
    ...
    if cmdargs.transfer == 'imagenet':
        base_model = (
            ...
            f"_g({GEN_PRESETS[cmdargs.gen]['model']})_bert-{cmdargs.gen}-512"
        )

    if cmdargs.transfer == 'self':
        base_model = (
            'endoscopy/model_d(cyclegan)_m(autoencoder)_d(None)'
            f"_g({GEN_PRESETS[cmdargs.gen]['model']})_bert-{cmdargs.gen}-512"
        )
...
# args
    'label'  : (
        ...
        f'-{cmdargs.loss}-{cmdargs.gp}-{cmdargs.cycle}-512'
    ),
    'outdir' : os.path.join(ROOT_OUTDIR, 'endoscopy'),
    'log_level'  : 'DEBUG',
    'checkpoint' : 50,
